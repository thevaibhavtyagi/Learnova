...
describe("imagesService helpers", () => {
  ...
  test("validateFaceDescriptor validates correct length and formats", () => {
    ...
    expect(() => validateFaceDescriptor(123)).toThrow("Invalid face descriptor format");

    // Payload too large
    const longString = "A".repeat(25000);
    expect(() => validateFaceDescriptor(longString)).toThrow("Face descriptor payload too large");

    // Invalid array length (e.g. 10 instead of 128)
    const shortArray = Array(10).fill(0.5);
    expect(() => validateFaceDescriptor(JSON.stringify(shortArray))).toThrow("Invalid face descriptor format");
